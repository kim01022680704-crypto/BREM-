...
app.post('/api/admin/riders/merge-duplicates', async (req, res) => {
  try {
    const result = await ridersAdmin.mergeDuplicateRiders(getBearerToken(req), {
    ...
    res.status(500).json({ error: error.message || '중복 기사 병합에 실패했습니다.' });
  }
});
