...
const riderMaintenance = require('./rider-maintenance');
...
app.get('/api/rider/maintenance', async (req, res) => {
  try {
    const result = await riderMaintenance.listMine(getBearerToken(req));
    if (!result.ok) return res.status(result.status || 400).json({ error: result.error });
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message || '정비기록을 불러오지 못했습니다.' });
  }
});

app.post('/api/rider/maintenance/bike', async (req, res) => {
  try {
    const result = await riderMaintenance.saveBike(getBearerToken(req), req.body || {});
    if (!result.ok) return res.status(result.status || 400).json({ error: result.error });
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message || '오토바이를 저장하지 못했습니다.' });
  }
});

app.post('/api/rider/maintenance/log', async (req, res) => {
  try {
    const result = await riderMaintenance.saveLog(getBearerToken(req), req.body || {});
    if (!result.ok) return res.status(result.status || 400).json({ error: result.error });
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message || '정비일지를 저장하지 못했습니다.' });
  }
});
...
app.get('/api/admin/payroll/withdrawable-drivers', async (req, res) => {
  try {
    const result = await riderWithdrawal.listWithdrawableDrivers(
      getBearerToken(req),
      req.query.weekStart
    );
    if (!result.ok) {
    ...
    res.status(500).json({ error: error.message || '기사별 출금가능금액을 불러오지 못했습니다.' });
  }
});

app.post('/api/admin/payroll/withdrawal-requests/admin-create', async (req, res) => {
  try {
    const result = await riderWithdrawal.adminCreateWithdrawalForDriver(
      getBearerToken(req),
      req.body || {}
    );
    if (!result.ok) {
    ...
    res.status(500).json({ error: error.message || '관리자 출금 처리에 실패했습니다.' });
  }
});
...
app.post('/api/admin/payroll/withdrawal-requests/bulk-complete', async (req, res) => {
  try {
    const result = await riderWithdrawal.completeWithdrawalRequestsBulk(
      getBearerToken(req),
      req.body?.ids
    );
    if (!result.ok) {
    ...
    res.status(500).json({ error: error.message || '선택 출금완료 처리에 실패했습니다.' });
  }
});
...
app.post('/api/admin/baemin-delivery/partner-set-count', async (req, res) => {
  try {
    const body = req.body || {};
    const result = await baeminDeliveryCollect.savePartnerSetCountEntry(getBearerToken(req), {
      partnerId: body.partnerId,
    ...
    res.status(500).json({ error: error.message || '세트수 저장에 실패했습니다.' });
  }
});

app.get('/api/admin/baemin-delivery/partner-set-count', async (req, res) => {
  try {
    const result = await baeminDeliveryCollect.getPartnerSetCountMap(getBearerToken(req));
    if (!result.ok) {
      return res.status(result.status || 400).json({
        error: result.error || result.message,
        message: result.message || result.error
    ...
    res.status(500).json({ error: error.message || '세트수를 불러오지 못했습니다.' });
  }
});
...
app.get('/api/admin/baemin-delivery/view-full-bundle', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error.message || '배민현황 전체 데이터를 불러오지 못했습니다.' });
  }
});

app.get('/api/admin/baemin-delivery/view-rider-range', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error.message || '라이더별 배달내역을 불러오지 못했습니다.' });
  }
});
...
app.get('/api/admin/baemin-delivery/rider-collect-range', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error.message || '라이더 수집 기간을 불러오지 못했습니다.' });
  }
});

app.post('/api/admin/baemin-delivery/rider-collect-range', async (req, res) => {
  try {
    const result = await baeminDeliveryCollect.saveRiderCollectRange(getBearerToken(req), req.body || {});
    if (!result.ok) {
      return res.status(result.status || 400).json({
        error: result.error || result.message,
        message: result.message || result.error
    ...
    res.status(500).json({ error: error.message || '라이더 수집 기간 저장에 실패했습니다.' });
  }
});
...
app.post('/api/admin/baemin-delivery/live-accept-rates/rebuild', async (req, res) => {
  try {
    const body = req.body || {};
    const result = await baeminDeliveryCollect.rebuildLiveAcceptRates(getBearerToken(req), {
      weekStart: body.weekStart
    });
    if (!result.ok) {
      return res.status(result.status || 400).json({
