...
const BremAdminLeaseMenus = (function () {
  ...
  /** 주차 라벨만 만든다. state.paymentWeekStart 를 바꾸지 않는다. */
  ...
  function paymentConfirmMemo(weekStart, contractId = '') {
    const week = String(weekStart || '').slice(0, 10);
    const cid = String(contractId || '').trim();
    return cid
      ? `${PAYMENT_CONFIRM_MEMO_PREFIX}${week}:${cid}`
      : `${PAYMENT_CONFIRM_MEMO_PREFIX}${week}`;
  }

  function paymentConfirmMemoContractId(memo) {
    const text = String(memo || '');
    if (!text.startsWith(PAYMENT_CONFIRM_MEMO_PREFIX)) return '';
    const rest = text.slice(PAYMENT_CONFIRM_MEMO_PREFIX.length);
    const parts = rest.split(':');
    // 납부확인:YYYY-MM-DD:contractId
    return parts.length >= 2 ? String(parts[1] || '').trim() : '';
  }

  function findWeekPaymentConfirm(vehicleId, weekStart, contractId = '') {
    if (!erp() || !weekStart) return null;
    const week = String(weekStart).slice(0, 10);
    const cid = String(contractId || '').trim();
    ...
    if (cid) {
      const exactMemo = paymentConfirmMemo(week, cid);
      const byContract = list.find(item => {
        const memo = String(item.memo || '');
        if (memo === exactMemo) return true;
        if (!memo.startsWith(PAYMENT_CONFIRM_MEMO_PREFIX)) return false;
        if (paymentConfirmMemoContractId(memo) !== cid) return false;
        return String(item.dueDate || '').slice(0, 10) === week
          || memo.startsWith(`${PAYMENT_CONFIRM_MEMO_PREFIX}${week}`);
      });
      if (byContract) return byContract;
    }

    if (!vid) return null;
    // 예전 기록(메모에 계약ID 없음): 차량+주차로 매칭
    return list.find(item => {
      if (String(item.vehicleId || '') !== vid) return false;
      const memo = String(item.memo || '');
      if (!memo.startsWith(PAYMENT_CONFIRM_MEMO_PREFIX)) {
        return String(item.dueDate || '').slice(0, 10) === week && memo.includes('납부확인');
      }
      // 계약ID가 다른 메모는 제외 (같은 차량 재계약 대비)
      const memoCid = paymentConfirmMemoContractId(memo);
      if (memoCid && cid && memoCid !== cid) return false;
      if (memo.startsWith(`${PAYMENT_CONFIRM_MEMO_PREFIX}${week}`)) return true;
      return String(item.dueDate || '').slice(0, 10) === week;
    }) || null;
  }
  ...
  function paymentConfirmStatus(contract, weekStart) {
    ...
    const payment = findWeekPaymentConfirm(contract?.vehicleId, weekStart, contract?.id);
    if (!payment) {
    ...
    const normalCode = String(BremLeaseProfit?.PAYMENT_STATUSES?.NORMAL || 'normal').toLowerCase();
    ...
    // 완납으로 찍혔는데 금액 필드가 비어 있던 예전 기록
    if (payStatus === normalCode && paid > 0) {
      return { code: 'paid', label: '완납', cls: 'lease-status--done' };
    }
    return { code: 'pending', label: '확인대기', cls: 'lease-status--collecting' };
  }

  function upsertWeekPaymentConfirm({
    vehicleId,
    weekStart,
    chargeAmount,
    paidAmount,
    ...
    const week = String(weekStart || '').slice(0, 10);
    const cid = String(contractId || '').trim();
    const existing = findWeekPaymentConfirm(vehicleId, week, cid);
    ...
    const payload = {
      vehicleId: String(vehicleId || existing?.vehicleId || '').trim(),
      dueDate: week,
      ...
      memo: paymentConfirmMemo(week, cid || paymentConfirmMemoContractId(existing?.memo))
    };
  ...
  function markContractWeekFullyPaid(contract, weekStart) {
    if (!contract?.id || !weekStart) return null;
  ...
  /** 납부확인 미납 → 차감관리(홀드 장부) 생성/갱신. ERP차감 ON이면 이중 방지로 반영 OFF */
  function ensurePaymentConfirmUnpaidLedger({ contract, arrear, weekStart, unpaidAmount, unpaidDays }) {
    const store = window.BremStorage?.deductionLedger;
    if (!store || !contract || !arrear) return null;
    const amount = Math.max(0, Math.round(Number(arrear.unpaidAmount || unpaidAmount || 0)));
    if (amount <= 0) return null;
    const sourceRef = `payment-confirm:${contract.id}:${weekStart}`;
    const identity = contractDriverIdentity(contract);
    const erpOn = isContractFinalApplyEnabled(contract);
    const daily = Math.max(0, Math.round(Number(contractRiderDailyRent(contract) || 0))) || amount;
    const platform = String(contract.deductionPlatform || contract.rawData?.deductionPlatform || 'coupang') === 'baemin'
      ? 'baemin'
      : 'coupang';
    const existingId = String(arrear.rawData?.ledgerId || '').trim();
    ...
    const saved = store.save({
      ...
      driverName: identity.driverName || contract.driverName || '',
      driverPhone: identity.driverPhone || contract.driverPhone || '',
      dailyDeduct: daily,
      balance: amount,
    ...
    erp().arrears().update(arrear.id, {
      collectionMethods: [...new Set([
        ...(arrear.collectionMethods || []),
        'salary_deduction',
        'separate_deposit'
      ])],
      rawData: {
  ...
  /** 미납 잔액을 미납/회수 + 차감관리에 등록(누적)하고 미납/회수 메뉴로 이동 */
  async function moveRemainingToArrears({ contract, weekStart, unpaidDays, unpaidAmount, paidAmount, navigate = true }) {
    const completed = calc().ARREAR_STATUS.COMPLETED;
    ...
    if (openForContract) {
      const weekEntries = Array.isArray(openForContract.rawData?.weekEntries)
        ...
        : [];
      weekEntries.push(weekEntry);
      arrear = erp().arrears().update(openForContract.id, {
        unpaidDays: Number(openForContract.unpaidDays || 0) + unpaidDays,
        unpaidAmount: Number(openForContract.unpaidAmount || 0) + unpaidAmount,
        paidAmount: Number(openForContract.paidAmount || 0) + Number(paidAmount || 0),
        collectionMethods: [...new Set([
          ...(openForContract.collectionMethods || []),
          'salary_deduction',
          'separate_deposit'
        ])],
        collectionStatus: collecting,
    ...
    ensurePaymentConfirmUnpaidLedger({
      contract,
      arrear,
      weekStart,
      unpaidAmount,
  ...
  async function confirmPaymentFull(contractId, options = {}) {
    ...
    if (!options.silent) {
      showToast(`완납 확인 · ${formatMoney(result.charge)} · 이 주 미확인에서 제외`);
      state.paymentConfirmSelectedIds.delete(String(contractId));
      // 미확인 탭에 남아 바로 목록에서 빠지게 함 (완납 탭·완납확인에서 조회)
  ...
  async function bulkConfirmPaymentFull() {
    ...
    state.paymentStatusFilter = 'open';
    showToast(failCount
      ? `완납 ${okCount}건 · 실패 ${failCount}건`
      : `선택 ${okCount}건 완납 · 이 주 미확인에서 제외`);
    renderPaymentConfirm();
    renderPaymentPaid();
    refreshAfterLeaseMutation({ contract: false });
  }
  ...
  async function confirmPaymentUnpaid(contractId) {
    ...
    try {
      ...
      updateLeaseErpUnsavedBanner();
      state.paymentConfirmSelectedIds.delete(String(contractId));
      // 미확인에 남아 바로 목록에서 빠지게 함 (미납 탭에서 조회)
      state.paymentStatusFilter = 'open';
      renderPaymentConfirm();
      refreshAfterLeaseMutation({ contract: false });
      showToast(`미납 확인 · ${formatMoney(charge)} · 이 주 미확인에서 제외`);
    } catch (error) {
  ...
  function syncPaymentConfirmTabUi() {
    ...
    let status = state.paymentStatusFilter;
    if (status === 'partial') status = 'unpaid';
    if (!['open', 'unpaid', 'paid'].includes(status)) status = 'open';
  ...
  function matchesPaymentStatusFilter(statusCode, filter) {
    ...
    if (filter === 'unpaid' || filter === 'partial') {
      return statusCode === 'unpaid' || statusCode === 'partial';
    }
    // open = 미확인(확인대기만)
  ...
  function renderPaymentConfirm() {
    ...
    if (source === 'loan') {
      ...
      if (summaryEl) {
        const statusLabel = statusFilter === 'paid' ? '완납' : (statusFilter === 'unpaid' || statusFilter === 'partial' ? '미납' : '미확인');
      ...
      rowsEl.innerHTML = loans.map(loan => {
        ...
        const actions = status.code === 'paid'
          ...
          : `<button type="button" class="small-btn primary-btn" data-loan-payment-full="${escapeHtml(loan.id)}">완납</button>
             <button type="button" class="small-btn" data-loan-payment-unpaid="${escapeHtml(loan.id)}">미납</button>`;
        const interest = Math.max(0, Math.round(Number(loan.interest || 0)));
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', event => {
      ...
      const loanPaymentUnpaid = event.target.closest('[data-loan-payment-unpaid]');
      if (loanPaymentUnpaid) {
        void confirmLoanPaymentUnpaid(loanPaymentUnpaid.dataset.loanPaymentUnpaid);
        return;
      }
      ...
      if (paymentStatusBtn) {
        ...
        const normalized = next === 'partial' ? 'unpaid' : next;
        state.paymentStatusFilter = ['open', 'unpaid', 'paid'].includes(normalized) ? normalized : 'open';
        state.paymentConfirmSelectedIds.clear();
        renderPaymentConfirm();
        return;
      }
