(function () {
  ...
  const COL = Object.freeze({
    ...
    excelWithholdingTax: 18,    // S 원천세(프로모션원천세 포함)
  ...
  function readExcelRow(row) {
    return {
      ...
      jColumnAmount: 0,
      bremPromotion: parseMoney(cellValue(row, COL.bremPromotion)),
  ...
  /** 수~화 정산주 종료(화) 기준 기본 지급일 = 금요일 (+3일). 명절 분할주는 다가오는 지급일(배달료 먼저). */
  function defaultPaymentDateForWeek(weekStart) {
    const week = normalizeSettlementWeekStart(weekStart);
    const holidayPay = window.BremSettlementHoliday?.upcomingPaymentDate?.(week)
      || window.BremSettlementHoliday?.latestPaymentDate?.(week);
  ...
  function templateRows() {
    const header = ['', 'B 대리점명', 'C 기사명', '', '', 'F 콜수', 'G 배달료', 'H 추가지급', 'I 기타지급', '', 'K BREM프로모션', 'L 고용보험', '', 'N 산재보험', '', 'P 시간제보험', 'Q 콜수수료', 'R 콜수수료', 'S 원천세', 'T 실지급액'];
    const sample = ['', 'OO대리점', '홍길동', '', '', 120, 3500000, 50000, 30000, '', 40000, 15000, '', 8000, '', 5000, 50000, 7000, 120000, 3400000];
